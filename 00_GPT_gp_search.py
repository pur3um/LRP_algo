...
# -*- coding: utf-8 -*-
"""
BoTorch 4-parameter HPO for run_nerf_ranksched_final_testpsnr_ssim.py.

Key behavior:
  1) BO objective = final validation-set mean PSNR parsed from [HPO_VAL].
  2) Acquisition = qLogExpectedImprovement.
  3) GP = SingleTaskGP(input_transform=Normalize(d=4), outcome_transform=Standardize(m=1)).
...
def unit_to_params(x):
    """Map a point in [0, 1]^4 to the actual hyperparameter values."""
...
def propose_batch(train_x, train_y, q, seed):
    ...
    if train_x.ndim != 2 or train_x.shape[-1] != 4:
        raise ValueError(f"train_x must have shape n x 4, got {tuple(train_x.shape)}")
...
def save_summary(path, scene, results, train_scheduler=None, final_test_metrics=None):
    ...
    summary = {
        "scene": scene,
        "method": "BoTorch batch BO for run_nerf_ranksched_final_testpsnr_ssim.py",
        ...
        "gp_model": "SingleTaskGP(input_transform=Normalize(d=4), outcome_transform=Standardize(m=1))",
